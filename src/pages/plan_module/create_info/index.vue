  ...
  <div class="container">
    <div class="module">
      ...
      <div class="item">
        ...
        <div class="value">
          <input
            v-model="insurance.insurant_name"
            type="text"
      ...
      <div class="item none">
        ...
        <div class="value" style="justify-content: flex-end">
          <picker
            ...
            fields="day"
            @change="e => (insurance.insurant_birth = e.mp.detail.value)"
          >
    ...
    <div class="module" v-if="insurance.same_flag === 'Y'">
      ...
      <div class="item none">
        ...
        <div class="value" style="justify-content: flex-end">
          <picker
            ...
            fields="day"
            @change="e => (policy.policy_birth = e.mp.detail.value)"
          >
      ...
      <div class="item">
        <span class="label">国籍</span>
        <div class="value">
          <mx-picker label="description" valueKey="value" :data="template.countries" v-model="policy.policy_country" />
        </div>
        <img class="arrow" src="/images/icon_arrow_product.png">
      </div>
      <div class="item">
        <span class="label">居住地</span>
        <div class="value">
          <mx-picker label="description" valueKey="value" :data="template.lives" v-model="policy.policy_address" />
        </div>
        <img class="arrow" src="/images/icon_arrow_product.png">
      </div>
      <!--<div v-if="insuranceType === 'HONGKONG_WYSX'">
      ...
      <div class="item">
        <span class="label">是否吸烟</span>
        <div class="value">
          <mx-picker label="description" valueKey="value" :data="template.yesno" v-model="policy.policy_somking_flag" />
        </div>
        <img class="arrow" src="/images/icon_arrow_product.png">
      </div>
    </div>

    <div class="btn_next" @click="calculate">
      <span>提交</span>
    </div>
    ...
    <div class="confirm" v-if="showConfirm">
      ...
      <h3>保费：{{amount}}{{params.currency}}</h3>
      <div class="btn_group">
        <div class="cancel" @click="showConfirm = false">返回修改</div>
        <div class="submit" @click="submit">继续提交</div>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async getTemplate () {
      try {
        ...
        this.template.product_year_periods.forEach(item => {
          if (item.subline_id === parseInt(this.params.subline_id)) {
            this.sublineName = item.subline_item_name
          }
        })
      } catch (e) {
        throw new Error(e)
    ...
    async calculate () {
      try {
        ...
        this.plan = plan
        let data = {
          'coverage': this.params.amount,
          'pay_method': this.params.pay_method,
          'gender': this.insurance.insurant_gender,
          'smoke_flag': this.insurance.insurant_somking_flag,
          'currency': this.params.currency,
          'item_id': this.params.item_id,
          'security_level': this.params.security_level,
          'security_area': this.params.security_area,
          'selfpay': this.params.selfpay,
          'country': this.insurance.insurant_country,
          'subline_item_name': this.sublineName,
